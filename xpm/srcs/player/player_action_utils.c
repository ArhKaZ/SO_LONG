...
/*   Updated: 2023/03/13 17:03:32 by syluiset         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_map *map, t_player *player, t_gps *new)
{
	...
	if (player->coor->x == map->coor_exit->x && player->coor->y == map->coor_exit->y)
...
void	move_player_sprite(t_param *param, t_gps *new, int direction)
{
	...
	if (param->player->coor->x == param->map->coor_exit->x
		&& param->player->coor->y == param->map->coor_exit->y)
		sprite = param->textures->black_hole_close->bh[0]->p;
	else
